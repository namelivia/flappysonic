...
export default class Collision {
      ...
      checkPixelCollision(bitmap1, bitmap2) {
        ...
        imageData1 = this._intersectingImagePart(intersection,bitmap1,this.collisionCtx,1)
        imageData2 = this._intersectingImagePart(intersection,bitmap2,this.collisionCtx2,2)
    
        //compare the alpha values to the threshold and return the result
        // = true if pixels are both > 0 at one coordinate
        pixelIntersection = this._compareAlphaValues(imageData1,imageData2,intersection.width,intersection.height)
      ...
      _intersectingImagePart(intersetion,bitmap,ctx,i) {
        ...
        var frameName = frame.image.src + ':' + 
                      frame.rect.x + ':' + frame.rect.y + ':' + 
                      frame.rect.width  + ':' + frame.rect.height;
          if ( this.cachedBAFrames[frameName] ) {
            image = this.cachedBAFrames[frameName]
          } else {
            this.cachedBAFrames[frameName] = image = SpriteSheetUtils.extractFrame(bitmap.spriteSheet,bitmap.currentFrame)
          }
        }
      ...
      _getParentalCumulatedProperty(child,propName,operation) {
        ...
        if ( child.parent && child.parent[propName] ) {
          ...
          var pp = this._getParentalCumulatedProperty(child.parent,propName,operation)
          if ( operation == '*' ) {
            return cp * pp
          } else {
            return cp + pp
